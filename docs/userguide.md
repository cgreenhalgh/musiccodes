...
1. Raw notes are input from a selected MIDI input or from the default or a selected audio input. Each note has a pitch (frequency), volume (velocity) and onset (start) time; muzicodes does not use the note's duration.
...
1. Groups of notes are then matched against the defined codes (patterns), according to a specified matching profile. The matching profile specifies how delays (and pitches) are quantised, and for inexact matching specifies the specific "costs" or "errors" associated with differences between notes and codes. The matching profile can also specify note filtering and grouping for individual codes.
...
1. In the `2. Note Filter/grouping` tab (top panel) set minimum and/or maximum frequencies and velocities if your example includes spurious notes such as overtones. These settings are defaults for all codes; you can also set similar constraints in the matching profile ("1") for the code(s) which use that profile.
...
1. In the `Edit` tab press `Open master view` to open the player view (if it is already open press `Reload` in the player view).
...
The default audio input is normally used (as selected by system settings/control panel). The drop-down list in `Audio input` allows you to specify a particular audio input device. Note: you may need to quit and restart the browser if you change the physical audio devices. By default the first input channel (0) is used; an alternative channel can be selected with `Audio channel`. Note: Chrome currently only supports access to channels 0 and 1, even when the device has more channels.
...
Values specified here are default values used for all codes, and can be over-riden by specific matching profiles.
...
If you are playing polyphonic music then by default notes will appear in codes in the order in which they are detected by the system. This can give unpredictable results with simultaneous notes as the exact order of the notes may be different each time they are played, e.g. the notes of a chord may be detected in essentially random order. To avoid this set `minimum gap between chords ` to a small but non-zero value (e.g. 0.1 seconds); the system will then treat notes that are this close together in time as being at the "same" time, and sort them into ascending pitch. In addition you can choose to `keep all notes` detected at the "same" time (the default), or only the loudest or the lowest note of each chord.

If you do not care about the octave in which notes are played then set `Translate pitch in code` to `Always octave 4`; this will make all notes appear to be played in octave 4 when matching codes. If you do not care about the pitch at all then set it to `Always C4` and use only the note `C4` in codes: any note will appear to have this pitch.
...
You can also specify per-profile filter and grouping settings here. These values over-ride the default values specified in the `2. Note Filter/Grouping` tab.
...
- if so with how much "error"
- its priority, i.e. in what order codes will be matched
...
If you are using conditional matching and have checked `Show State in codes` in the conditional matching tab then you will also see the precondition rule for the code and the `post state`, i.e. changes to make to variables if the code is matched.

#### Controls tab (lower panel)

This allows you to control conditional matching variables from an external midi device.

Set the control midi input using the `Midi Control Input` drop-down. This can be the same as any midi note input, or a different midi input. 

lue), updates to be made to state if marker is triggeres (simultaneous assignment).

Values for `input Url` include:
- MIDI message (same syntax as code actions), starting `data:text/x-midi-hex,`, e.g. `data:text/x-midi-hex,903a7f` (note on 60 channel 1) 
- on load, `event:load`, which will trigger associated actions when this experience is first loaded
...
As with codes, control inputs can also have actions and (with conditional matching) preconditions and post state changes to variables.
